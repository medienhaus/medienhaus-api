...
import { PostService } from './item_post.service'
...
export class ItemService {
  constructor (configService, httpService) {
    ...
    this.PostService = new PostService(this.configService, this.httpService, this.matrixClient, this.items, this.structure, this.allSpaces, this._allRawSpaces)
  }
  ...
  convertSpace (id, space, maxDepth = 2, currentDepth = 0) {
    if (currentDepth >= maxDepth) {
      console.log(id)
      console.log(space?.descriptions)
      console.log('------------')
      return
    }
    ...
    if (id === '!xWDKFUDdrZnqXFZIJs:dev.medienhaus.udk-berlin.de') {
      console.log(space?.descriptions?.map((desc) =>
    ...
    return {
      ...
      type: space?.type,
      template: space?.template,
      item: _.reduce(space?.item, (ret, item) => { const e = this.convertSpace(item?.id, undefined, maxDepth, currentDepth + 1); if (e != null && e !== undefined && e !== '') { ret.push(item) };return ret }, []),
      context: _.reduce(space?.context, (ret, context) => { const e = this.convertSpace(context?.id, undefined, maxDepth, currentDepth + 1); if (e != null && e !== undefined && e !== '') { ret.push(context) };return ret }, []),
      content: _.map(space?.content, (content) =>
        this.convertSpace(content.id, undefined, maxDepth, currentDepth + 1)
      ),
